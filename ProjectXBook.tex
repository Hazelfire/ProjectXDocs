...
skipSplashes: true
	...
	You can enable fullscreen by setting \texttt{fullScreen} to \texttt{true} instead of \texttt{false}. The \texttt{skipSplashes} property is not shown by default. If set to true, then the 2 splashes at the start of the game will not show and it will take you directly to the menu page. This property is called a \textbf{hidden} property. To enable a hidden property, just add it to the end of your save file.
...
<class name="Rat Man">
	...
	In XML you can also have \textbf{comments}. Comments are pieces of code that are ignored, and are there to make it easier for people to read. A comment starts with \texttt{<!--} and ends with \texttt{-->}.
	
	\begin{xml}
		<!-- Hello, I am a comment and will be ignored! -->
	\end{xml}
	
	Usually, in well formatted XML, there is only one tag per file, for example, having these two items in the same file is considered bad practice, and they should be split across files.
...
<item name="Axe">
	...
	The sprites XML file contains all the different sprites that will be used in various other aspects of the game, those aspects being players, tiles, creatures and items.
	
	Inside \texttt{script/xml/sprites} there are 4 subdirectories which split up the different kinds of sprites. 
	\begin{lstlisting}
	script/xml/sprites
	├───creatures
	├───items
	...
	Here are examples of sprite xml files:
	
	\subsubsection{Items spritesheet}
	In the items spritesheets, the size is assumed to be 32x32, so it does not need to be specified.
	
	\texttt{Base/script/xml/sprites/items/Resources.xml}
	\begin{xml}
<spritesheet source = "Resources.png" >
	...
	\subsubsection{Tiles spritesheet}
	In the tiles spritesheet, the tiles are assumed to be in multiples of 32 wide and 24 tall. So we specify their height and width in terms of tiles instead of pixels.
	
	\texttt{Base/script/xml/sprites/tiles/Terrain.xml}
	\begin{xml}
<spritesheet source = "terrain.png" tileWidth = "1" tileHeight = "1" >
	...
	\subsubsection{Creatures spritesheet}
	In the creatures spritesheet, the height and width must be specified in pixels.
	
	\texttt{Base/script/xml/sprites/creatures/RatAnim.xml}
	\begin{xml}
<spritesheet source= "Rat anim.png" height= "10" width="16">
	...
	\subsubsection{Players spritesheet}
	The players spritesheet is similar to the creatures one. You need to give the height and width in pixels
	
	\texttt{Base/script/xml/sprites/player/Birdfoot.xml}
\begin{xml}
...
\texttt{Base/script/xml/music/Plains.xml}
\begin{xml}
<region name="plains">
...
\texttt{Base/script/xml/items/tools/Sword.xml}
\begin{xml}
<item name="Sword">
	<ingredients>
	Wood:1;
...
\texttt{Base/script/xml/items/resources/Apple.xml}
\begin{xml}
<item name="Apple">
	<description>
...
The interactions XML file tells the game what to do when different items are clicked on around the map.

All items that are on the bottom layer are assumed to be walkable unless specified. All items on the top layer are considered solid unless specified otherwise.

Here is a couple of examples:

\texttt{Base/script/xml/interactions/Water.xml}
\begin{xml}
<tile name="water" impassable="true" />
\end{xml}

\texttt{Base/script/xml/interactions/AppleTree.xml}
\begin{xml}
<tile name="appleTree">
	<action name="chop">giveAndDestroy("Wood", 3)</action>
	<action name="pick">me.giveItem("Apple")</action>
</tile>
\end{xml}

The file consists of a \texttt{tile} tag. Each tag represents an tile specified in the map generation XML file. The name attribute is the exact name of the tile. In the example of the Apple Tree. Each tile can have one or more actions associated with it. The text inside the action tag is Lua code that will be executed after the player has clicked and walked to the tile. In this case, clicking on the apple tree will ask the player which action they would like to do, and will give the player an apple if desired, or collect wood from the tree. The function \texttt{giveAndDestroy} is a custom one and not part of the standard functions, more information on this can be found at the Lua interactions section.

\hFigure{MultipleSelection.jpg}{Selection Wheel}

The water example shows how to make a tile impassible. All that needs to be done is to set the \texttt{impassable} attribute to true.

If there is only one action, it will be automatically run when the player clicks on it. The name attribute is not needed in this case. The lua code is run in the \textbf{interactions} scope.
...
<tile name="appleTree">
	<action name="chop">giveAndDestroy("Wood", 3)</action>
	<action name="pick">me.giveItem("Apple")</action>
	<action name="smash" if="me.getClass() == 'Hulk'">Arena:destroyTile(x, y)</action>
</tile>
\end{xml}

The movable tag at the top is for convenience, this tag lists all the tiles, separated by semicolons, that can be walked on.

\section{Players}
The Players XML file is where all the playable character classes are listed. Here is an example of the file:

\texttt{Base/script/xml/player/Birdfoot.xml}
\begin{xml}
<class name="Birdfoot">
	<movement>
...
\texttt{Base/script/xml/creatures/rats/Rat.xml}
\begin{xml}
...
\texttt{Base/script/xml/map/standard.xml (clipped)}
\begin{xml}
...
\texttt{Base/script/xml/schematics/Apple Cirle.xml}
\begin{xml}
...
function giveAndDestroy(itemName, quantity)
	collect(itemName, quantity, tilePos, 1);
end
\end{lua}

This function is found in  a script inside the interactions lua folder. The function collects the object from the ground, the collect function waits for the given amount of time (last argument in seconds) and then destroys the tile and gives the player the item. If you wanted to, you could define what all actions do inside the xml interactions file, but sometimes, like this giveAndDestroy, it is more practical to write a function for it then to write the code 4 times.
