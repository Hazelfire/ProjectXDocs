...
\begin{document}
	...
	Every single tile can have a state associated with it, a state is actually a number, but it is used to find out what has happened to the tile and some of it's properties. So far, there are 3 offial states, those states are untouched (0), Destroyed (1), Depleted(2). Any tile with a state of 1 (destoyed) will not be shown on the map.
	
	You can invent your own states and use the however you wish, for example, you may have 4: hanuted, which means that a ghost will spawn if the player tries to break it.
...
skipSplashes: true
	...
	\hFigure{Atom.png}{Screenshot of Atom}
	...
	The Lua terminal can excecute lua commands in game and is very helpful starting point for programming in Lua. It is similar to the minecraft terminal except that it will only accept commands and the commands are in lua. The Terminal can be opened with T and will only be available if you enable \texttt{devMode} inside \texttt{SaveFile.yml} (see Save file above). Escape will close the terminal.

	You can excecute commands from here (such as print("Hello world")) and the output (if there is any) will show above where you typed it. It will also show any lua errors that happened in the game, so you do not have to check your log file for errors, you can just press T to view it.
...
\hFigure{LuaScopeDiagram.png}{Lua Scopes}
...
This code gets called if the player has chosen to \textit{pick} from the tree, and the player has walked up to it. Simply, it just gives the player an Apple.

\part{Appendix}
\chapter{Appendix A - Lua functions}
This chapter outlines all the available Lua functions in a reference style manner, this is handy for knowing how to use a specific function. It is organised in Scope - Classes - Members
\section{Interpreter}
\subsection{Debug}
...
\begin{ulist}
	\item nil moveOn(number x, number y)
	\item nil moveOn(number x, number y, number speed)	
\end{ulist}
...
Overloads:
\begin{ulist}
	\item nil moveTo(number x, number y)
	\item nil moveTo(number x, number y, number distance)
	\item nil moveTo(number x, number y, number distance, number speed)
\end{ulist}
...
x and y are coordinates of the tile that is being clicked on.
